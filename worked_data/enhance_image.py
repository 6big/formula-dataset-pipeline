...
def apply_enhancements(image: np.ndarray, random_seed: Optional[int] = None) -> np.ndarray:
    """
    应用 ±5° 旋转增强
    ...
    # 计算新边界尺寸
    cos_angle: float = abs(rotation_matrix[0, 0])
    sin_angle: float = abs(rotation_matrix[0, 1])
    new_width: int = int((height * sin_angle) + (width * cos_angle))
    new_height: int = int((height * cos_angle) + (width * sin_angle))

    # 调整变换矩阵以保持中心
    rotation_matrix[0, 2] += (new_width / 2) - center[0]
    rotation_matrix[1, 2] += (new_height / 2) - center[1]

    enhanced: np.ndarray = cv2.warpAffine(
        image,
        rotation_matrix,
...
def _get_images_from_jsonl(dataset_jsonl: str) -> Set[str]:
    ...
    try:
        with open(dataset_jsonl, "r", encoding="utf-8") as file_handle:
            return {
        ...
        print(f"⚠️ 读取 JSONL 文件时出错: {error}")
        return set()
...
def enhance_images_in_place(
        ...
        random_seed: int = 42) -> str:  # 新增：随机种子
    """
    原地增强图片（直接替换原图）
    ...
    try:
        ...
        if not images_path.exists():
            result: str = f"❌ 图片目录不存在: {images_dir}"
            print(result)
            print("=== 函数执行完成 ===")
            sys.stdout.flush()
            return result
        ...
        # 支持的图像格式
        supported_formats: Set[str] = {".png", ".jpg", ".jpeg", ".bmp", ".tiff", ".gif"}

        # 获取要处理的图片列表
        if dataset_jsonl and Path(dataset_jsonl).exists():
            jsonl_images: Set[str] = _get_images_from_jsonl(dataset_jsonl)
            all_images: List[str] = []
            for file in images_path.iterdir():
                if (file.suffix.lower() in supported_formats and 
                    file.name in jsonl_images):
                    all_images.append(file.name)
            print(
                f"🔍 从 JSONL 读取到 {len(jsonl_images)} 个图片引用，目录中找到 {len(all_images)} 个匹配的图片"
            )
        else:
            all_images: List[str] = []
            for file in images_path.iterdir():
        ...
        if enhance_strategy == "all":
            indices_to_augment: Set[int] = set(range(total_images))
        elif enhance_strategy == "random":
            if num_to_augment is None:
                num_to_augment = int(total_images * augmentation_ratio)
            # 当num_to_augment为0时，不增强任何图片
            if num_to_augment == 0:
                indices_to_augment = set()
            else:
                indices_to_augment = set(
                    np.random.choice(
                        total_images, min(num_to_augment, total_images), replace=False
                    )
                )
        else:  # "deterministic"
            if num_to_augment is None:
            ...
            if num_to_augment == 0:
                indices_to_augment = set()
            elif total_images <= num_to_augment:
                indices_to_augment = set(range(total_images))
            else:
                step: float = total_images / num_to_augment
                indices_to_augment = set()
        ...
        for idx, filename in enumerate(all_images):
            ...
            apply_augmentation: bool = idx in indices_to_augment
            ...
            try:
                ...
                print(f"❌ 处理失败 {filename}: {error}")
                failed_count += 1

            # 进度显示
            if processed_count % 50 == 0:
                print(
                    f"📊 进度: {processed_count}/{total_images} (已增强: {augmented_count})"
                )
